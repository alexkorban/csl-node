...
class SyncHandlers
    ...
    addExtraResponseData: (responseData) =>
        ...
        extraQueries =
            ...
            projectOvernightStatus: @getIsActiveOvernight()

        Promise.props extraQueries
        .then (extraResponseData) =>
            R.merge responseData, (R.merge (R.omit "projectOvernightStatus", extraResponseData),
                project: R.merge extraResponseData.projectOvernightStatus, id: @projectCid)
    ...
    createUser: (items) =>
        ...
        @db.query """
            ...
            insert into users (name, phone_no, company, truck_no, truck_wheel_count, email, platform
                , cid, role_id, description)
            values ($1, $2, $3, $5, $6, lower($7), $8, $9
                -- role_id
                , case when exists(select * from role) then (select first(id) from role)
                     else (select id from user_roles where name = '') end
        ...
        , latest.email, (latest.platform ? "Android"), db.generateCid()
        .then (user) =>
    ...
    updateUser: (items) =>
        ...
        @db.query """
            update users set name = $1, phone_no = $2, company = $3, truck_no = $4, truck_wheel_count = $5
                , updated_at = clock_timestamp(), email = lower($6)
            where id = $7
        """
        , latest.name, latest.phoneNo, latest.company, latest.truckNo, latest.truckWheelCount, latest.email, @userId
        .then => null


    recordPushId: (items) =>
        # Apply the latest item only
        latest = R.last items

        @db.query """
            update users set push_id = $1
            where id = $2
        """
        , latest.pushId, @userId
        .then => null
    ...
    recordEvent: (data) =>
        ...
        Promise.all [entryExitEvents(), jhaEvents(), concreteMovementEvents(), motionEvents(),
            prestartChecklistEvent()]
        .then (results) ->
    ...
    # Incoming request handler for device queue items
    recordSignonEvent: (data) =>
        events = R.map ((item) ->
            item.properties = JSON.stringify item.properties
            item.position = if item.position? then JSON.stringify item.position else null
            item)
        , (db.renameKeysForDb data)

        @db.query """
            insert into signon_events(user_id, project_id, position, properties, created_at, vehicle_id)
                with event_records as (
                    select position::json, properties::json, created_at, vehicle_id
                    from json_populate_recordset(null::signon_event_json, $1)
                )
                select $2 as user_id, $3 as project_id, event_records.position, event_records.properties
                    , event_records.created_at
                    , (select id from vehicles where cid = vehicle_id) as vehicle_id
                from event_records
                join projects on projects.id = $3
                where projects.deleted_at = '-infinity'
                    and exists (select 1 from users where id = $2)
        ...
        @db.query """
            insert into beacon_events(user_id, project_id, type, beacon_id, created_at, properties, position
                ...
                with event_records as (
                    ...
                    from json_populate_recordset(null::beacon_event_json, $1)
                )
                select $2 as user_id, project_id,
                    event_records.type, beacons.id, event_records.created_at, event_records.properties
                    , case
                        when event_records.position is null then null
                        when exists (select id from projects where id = project_id
                            and ST_Contains(download_boundary,
                ...
                from event_records
                    join beacons on event_records.beacon_id = beacons.cid
                    join projects on project_id = projects.id
                where projects.deleted_at = '-infinity'
                    and (event_records.type = 'entry' or event_records.type = 'exit')
    ...
    recordObservation: (observations) =>
        ...
        @db.query """
            insert into geometries(overlay_id, created_at, properties, geometry, user_id)
                select (select id from overlays where name = 'User defined areas' and project_id = projects.id) as overlay_id
                    ...
                    , ST_SetSRID(ST_Point((items->>'lon')::double precision, (items->>'lat')::double precision), 4326)
                        as geometry
                    , $1
                from json_array_elements($2) as items
                    join geometries
                        on ST_Contains(geometries.geometry, ST_SetSRID(ST_Point((items->>'lon')::double precision
                            , (items->>'lat')::double precision), 4326))
                    join overlays on overlay_id = overlays.id
                    join projects on project_id = projects.id
                where (overlays.properties->>'is_boundary')::bool and projects.deleted_at = '-infinity'
        """
        , @userId, JSON.stringify db.renameKeysForDb observations
        .then => null
    ...
    recordVehicle: (vehicles) =>
        makeVehiclePromises = (vehicle) =>
            updatePromise = @db.query """
                update vehicles
                set rego_exp_date = $1, mileage = $2, updated_at = $3, number = $4
                where cid = $5
            """
            , vehicle.regoExpDate, vehicle.mileage, vehicle.createdAt, vehicle.number, vehicle.id
            ...
            insertPromise =
                if !@projectId?
                    ...
                    @db.query """
                        insert into vehicles (cid, number, mileage, rego, rego_exp_date, make, model, created_at
                            , customer_id, role_id)
                        with vehicle_data as (
                            select number, mileage, rego, rego_exp_date, make, model, created_at
                            from json_populate_record(null::vehicles, $2)
                        )
                        select $4, vehicle_data.*, (select customer_id from projects where id = $1) as customer_id, 1
                        from vehicle_data
                        where not exists (select 1 from vehicles where cid = $3)
                    """
                    , @projectId, JSON.stringify(db.renameKeysForDb vehicle), vehicle.id
                    , db.generateCid()
...
#                throw new Error "Sync record for user #{@userId}/project #{@projectId} not found"
    ...
    getIsActiveOvernight: =>
        @logs.messages.push "Getting if project is active overnight"
        @db.jsonQuery """
            select (properties->>'is_active_overnight')::boolean as is_active_overnight from projects where id = $1
        """, @projectId
        ...
        params.queryAdded = (syncedAt) =>
            ...
            .then (overlays) =>
                Promise.all R.map (overlay) =>
                    ...
                    @db.jsonArrayQuery """
                        select geometries.id, geometries.type, geometries.properties
                            , ST_AsGeoJSON(geometry)::json as geometry
                            , to_char(geometries.created_at at time zone
                                (select timezone from projects where id = $1), 'DD/MM/YY HH24:MI:SS')
                                as created_at
                            , users.name as created_by
                        from geometries
                        left join users on users.id = user_id
                        where geometries.overlay_id = $2 and geometries.deleted_at = '-infinity'
                    """
                    , @projectId, overlay.id
                    .then (geometries) =>
    ...
    getUpdatedVehicles: =>
        ...
        params.queryAdded = (syncedAt) =>
            @db.jsonArrayQuery """
                    select vehicles.cid as id, number, rego, rego_exp_date, mileage, make, model
                        , beacons.cid as beacon_id, vehicle_roles.name as role
                        , to_char(vehicles.updated_at at time zone (select timezone from projects where id = $1),
                            'DD/MM/YY HH24:MI:SS') as updated_at
                    from vehicles
                    left join beacons on beacons.id = vehicles.beacon_id
                    left join vehicle_roles on vehicle_roles.id = vehicles.role_id
                    where vehicles.customer_id = (select customer_id from projects where id = $1) and vehicles.updated_at >= $2
                        and vehicles.deleted_at = '-infinity'
                """
            , @projectId, syncedAt
    ...
    getUpdatedBeacons: =>
        ...
        params.queryAdded = (syncedAt) =>
            @db.jsonArrayQuery """
                select beacons.cid as id, beacon_uuid, major, minor, beacons.name, description
        ...
        params.queryDeleted = (syncedAt) =>
            @db.jsonArrayQuery """
                select cid as id from beacons where project_id = $1 and deleted_at > $2
            """
            , @projectId, syncedAt

        params.queryHash = =>
            @db.jsonArrayQuery """
                select cid as id, to_char(updated_at at time zone (select timezone from projects where id = $1),
                    'DD/MM/YY HH24:MI:SS') as updated_at
                from beacons
                where project_id = $1 and deleted_at = '-infinity'
            """
...
module.exports = (helpers) ->
    sync: helpers.withErrorHandling (req, res) ->
        ...
        .then (result) =>
            handlers.handleRequest("recordPushId", result)
        .then (result) =>
            handlers.handleRequest("requestFullSet", result)
        .then (result) =>
            handlers.getLastSyncTime().then => result  # Get the last sync time BEFORE we update it with current time
        .then (result) =>
        ...
        .then (result) =>
            handlers.handleRequest("recordSignonEvent", result)
        .then (result) =>
    ...
    notifications:
        helpers.withErrorHandling (req, res) ->
            req.logs.messages.push "Push notification message for #{req.params.id}: #{req.body.message}"

            req.db.master.jsonQuery """
                select push_id, platform
                from users
                where id = $2 and exists (select 1 from users_projects where user_id = $2 and project_id = $1)
            """
            , req.params.projectId, req.params.id
            .then (user) ->
                req.logs.messages.push "Got user #{JSON.stringify user}"
                if !user.pushId?
                    Promise.resolve null
                else
                    aws.config.update
                    ...
                    sns = new aws.SNS()
                    createPlatformEndpoint = Promise.promisify(sns.createPlatformEndpoint, sns)
                    publish = Promise.promisify(sns.publish, sns)

                    req.logs.messages.push "Configured SNS"

                    applicationArn = if user.platform == "iOS"
                        process.env.SNS_IOS_APPLICATION_ARN
                    else
                        process.env.SNS_ANDROID_APPLICATION_ARN

                    createPlatformEndpoint PlatformApplicationArn: applicationArn, Token: user.pushId
                    .then (data) ->
                        req.logs.messages.push "Got platform endpoint"
                        ...
                        androidPayload =
                            ...
                            data:
                                ...
                                notId: Math.round(Date.now() / 1000)   # This needs to be unique to avoid notification
                                                                       # collapsing

                        # SNS requires that the payload for each platform is stringified JSON, and then the whole
                        # payload is subsequently stringified
                        payload =
                            default: req.body.message
                            APNS: JSON.stringify iosPayload
                            GCM: JSON.stringify androidPayload

                        publish Message: JSON.stringify(payload), MessageStructure: "json", TargetArn: endpointArn
            .then (data) ->
                req.logs.messages.push "Published to platform endpoint"
                res.json data
